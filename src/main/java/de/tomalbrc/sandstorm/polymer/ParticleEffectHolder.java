...
public class ParticleEffectHolder extends ElementHolder implements ParticleComponentHolder {
            ...
            for (Map.Entry<String, EventSubpart> entry : this.effectFile.effect.events.entrySet()) {
                if (entry.getKey().equals(event)) {
                    ...
                    for (EventSubpart subpart : list) {
                        if (subpart.log != null) {
                            Sandstorm.LOGGER.info("Particle event log: {}", subpart.log);
                        }
    ...
    private void updateRuntimePerEmitter(MolangRuntime runtime) throws MolangRuntimeException {
        ...
        if (looping != null) {
            ...
            if (this.age * Sandstorm.TIME_SCALE >= max) {
                // todo: check loop delay
                this.canEmit = false;
                if ((this.age+sleepTime) * Sandstorm.TIME_SCALE >= max) {
                    this.age = 0;
                    this.canEmit = true;
                }
            }
        ...
        var rt = runtime.edit();
        rt.setVariable("emitter_age", (float)this.age * Sandstorm.TIME_SCALE);
    ...
    public void tick() {
        ...
        try {
            ...
            if (emitterLifetimeEvents != null && emitterLifetimeEvents.timeline != null) {
                List<String> events = emitterLifetimeEvents.timeline.getEventsInRange(this.age* Sandstorm.TIME_SCALE, Sandstorm.TIME_SCALE);
    ...
    private void emitOrRemoveParticles() throws MolangRuntimeException {
        ...
        if (steady != null) {
            ...
            double spawnRateTick = runtime.resolve(this.get(ParticleComponents.EMITTER_RATE_STEADY).spawnRate) * Sandstorm.TIME_SCALE;
